...
typedef string transactionName_t;//가독성을 위해 계좌명 변수를 생성할 때 string 대신에 사용
typedef int won_t;//가독성을 위해 원 변수를 생성할 때 int 대신에 사용
typedef string name_t;//가독성을 위해 이름을 생성할 때 string 대신 사용
...
enum DayOfTheWeek {
    ...
    Monday = 0,
    Tuesday, Wednesday, Thursday, Friday, Saturday, Sunday//Enumd을 통해 일일이 선언하지 않아도 월요일부터 일요일에 0~6까지 자동 선언
...
public:
    ...
    // Destructor
    ~BankAccount() {
        cout << clientName << "님의 계좌가 삭제되었습니다. Account ID: ";//계좌 삭제를 위한 소멸자
        printf("%04d\n", accountID);
